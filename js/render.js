...
function renderTable(filtered,q){
  ...
  filtered.forEach(function(p,i){
    ...
    rows.push(
      ...
      '<td class="td-price price-srp">'+fmt(p.srp)+pcBadge(p,'SRP')+'</td>'+
      '<td class="td-price price-dp">'+fmt(window.DEALER_MODE?p.dp_std:p.dp)+pcBadge(p,'DP')+'</td>'+
    ...
    return '<div class="grid-card" onclick="openModal(\''+String(p.item_code).replace(/\\/g,'\\\\').replace(/'/g,"\\'")+'\')">'+
      ...
      '<div class="grid-body">'+
        ...
        '<div class="grid-prices">'+
          '<div class="grid-price"><div class="plabel" style="opacity:.75">SRP</div><div class="pval price-srp" style="font-size:.7rem;opacity:.8">'+fmtS(p.srp)+'</div>'+pcBadge(p,'SRP')+'</div>'+
          '<div class="grid-price"><div class="plabel" style="color:var(--dp);font-weight:600">'+(window.DEALER_MODE?'DP':'Dealer ₱')+'</div><div class="pval price-dp" style="font-size:.85rem;font-weight:700">'+fmtS(window.DEALER_MODE?p.dp_std:p.dp)+'</div>'+pcBadge(p,'DP')+'</div>'+
...
function openModal(itemCode){
  ...
  document.getElementById('modal-prices').innerHTML=
    '<div class="modal-price"><div class="plabel" style="opacity:.75">SRP</div><div class="pval price-srp">'+fmt(p.srp)+'</div>'+pcBadge(p,'SRP',true)+'</div>'+
    '<div class="modal-price dp-highlight"><div class="plabel" style="color:var(--dp)">Dealer Price</div><div class="pval price-dp">'+fmt(window.DEALER_MODE?p.dp_std:p.dp)+'</div>'+pcBadge(p,'DP',true)+'</div>'+
